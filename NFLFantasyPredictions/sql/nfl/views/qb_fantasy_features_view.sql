        ...
        (SELECT 
            ...
            WHERE
                ((`offense`.`player` = `of`.`player`)
                    ...
                    AND (`games`.`wk` < `g`.`wk`)
                    AND (`games`.`wk` >= 1))) AS `accumulated_player_py`,
        (SELECT 
                COALESCE(SUM(`offense`.`tdp`), 0)
            FROM
                (`offense_fantasy` `offense`
                JOIN `games` ON ((`offense`.`gid` = `games`.`gid`)))
            WHERE
                ((`offense`.`player` = `of`.`player`)
                    AND (`games`.`season` = `g`.`season`)
                    AND (`games`.`wk` < `g`.`wk`)
                    AND (`games`.`wk` >= 1))) AS `accumulated_player_tdp`,
        (SELECT 
                COALESCE(AVG(`offense`.`py`), 0)
            FROM
                (`offense_fantasy` `offense`
                JOIN `games` ON ((`offense`.`gid` = `games`.`gid`)))
            WHERE
                ((`offense`.`player` = `of`.`player`)
                    AND (`games`.`season` = `g`.`season`)
                    AND (`games`.`wk` < `g`.`wk`)
                    AND (`games`.`wk` >= (`g`.`wk` - 3)))) AS `last_3_games_avg_player_py`,
        (SELECT 
                COALESCE(AVG(`offense`.`tdp`), 0)
            FROM
                (`offense_fantasy` `offense`
                JOIN `games` ON ((`offense`.`gid` = `games`.`gid`)))
            WHERE
                ((`offense`.`player` = `of`.`player`)
                    AND (`games`.`season` = `g`.`season`)
                    AND (`games`.`wk` < `g`.`wk`)
                    AND (`games`.`wk` >= (`g`.`wk` - 3)))) AS `last_3_games_avg_player_tdp`,
        (CASE
            WHEN
                (`g`.`v` = `of`.`team`)
            THEN
