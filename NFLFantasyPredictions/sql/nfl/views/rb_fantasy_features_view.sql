        ...
        (SELECT 
            ...
            WHERE
                ((`offense`.`player` = `of`.`player`)
                    ...
                    AND (`games`.`wk` < `g`.`wk`)
                    AND (`games`.`wk` >= (`g`.`wk` - 3)))) AS `last_3_games_avg_player_ry`,
        (SELECT 
        ...
        (SELECT 
            ...
            WHERE
                ((`offense`.`player` = `of`.`player`)
                    AND (`games`.`season` = `g`.`season`)
                    AND (`games`.`wk` < `g`.`wk`)
                    AND (`games`.`wk` >= (`g`.`wk` - 3)))) AS `last_3_games_avg_player_tdr`,
        (SELECT 
